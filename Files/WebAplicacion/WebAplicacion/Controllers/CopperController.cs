...
namespace WebAplicacion.Controllers
{
    public class CopperController : Controller
    {
        ...
        public JsonResult GraficoBancos(DateTime fecha)
        {
            ...
            //TempData["totBanco"] = totalBancos;
            //ViewBag.TotalBancos = totalBancos;
            // return Json(new Response {Action=Accion, Message = Mensaje }, JsonRequestBehavior.AllowGet );
            return Json(chartData);
        }
       
        [HttpPost]
        public JsonResult GraficoCosto(DateTime fecha)
        {
            var fechaCompleto = fecha.ToShortDateString();
            var dia = fecha.ToString("dd");
            var mes = fecha.ToString("MM");
            ...
            string sConnString = ConfigurationManager.ConnectionStrings["ConexionDB"].ConnectionString;
            String query = queryBase(año, mes, fechaCompleto) +
                           " select  NombreCuenta,Sum(Debe_Dol - Habe_Dol) as total from #tmpAsientos " +
                           "  where IdRepGerencia = '001' and IdGruGerencia = '020' and idMes =" + mes + "  " +
                           "  group by idcuenta,cuenta,NombreCuenta ";
                ...
                using (SqlCommand cmd = new SqlCommand(query))
                {
                    ...
                    using (SqlDataReader sdr = cmd.ExecuteReader())
                    {
                        ...
                        while (sdr.Read())
                        {
                            chartData.Add(new object[]
                            {
                                sdr["NombreCuenta"], sdr["Total"],Convert.ToString(sdr["Total"]+" USD")
        ...
        [HttpPost]
        public JsonResult GraficoCosto632(DateTime fecha)
        {
            var fechaCompleto = fecha.ToShortDateString();
            var dia = fecha.ToString("dd");
            var mes = fecha.ToString("MM");
        ...
        [HttpPost]
        public JsonResult GraficoCosto601(DateTime fecha)
        {
            var fechaCompleto = fecha.ToShortDateString();
            var dia = fecha.ToString("dd");
            var mes = fecha.ToString("MM");
        ...
        public JsonResult GuardarImagen(GraficosCopper objGrafico)
        {
            ...
            if (objGrafico.GraficoCostos.Trim() != string.Empty)
            {
                SaveImage(objGrafico.GraficoCostos, "GraficoCostos");
            }

            if (objGrafico.GraficoCostos632.Trim() != string.Empty)
            {
                SaveImage(objGrafico.GraficoCostos632, "GraficoCostos632");
            }
            if (objGrafico.GraficoCostos601.Trim() != string.Empty)
            {
                SaveImage(objGrafico.GraficoCostos601, "GraficoCostos601");
            }
        ...
        {     
            ...
            string ruta = Server.MapPath("~/FolderCopper");
            string resp3 = correo.EnviarCorreoMineraCopper(ruta);
        ...
        public string queryBase(string año,string mes, string fechaCompleto)
        {
            string query = " Select a.ano_codano as Año, a.mes_codmes as IdMes, " +
                  ...
                  " Where a.cia_codcia=1 and a.ano_codano=" + año + " and a.mes_codmes<='" + mes + "' and CONVERT(date,b.adc_feccon, 103)<=CONVERT(date,'" + fechaCompleto + "', 103)  " +
                  " ";
        ...
        public void SaveImage(string base64, string nombre)
        {
            ...
            if (base64 != "")
            {
                ...
                string filepath = Path.Combine(Server.MapPath("~/FolderCopper"), nombre + ".png");
        ...
        #region Estos metodos no se usan

        [HttpPost]
        public JsonResult GraficoInventarios(DateTime fecha)
        {
            var fechaCompleto = fecha.ToShortDateString();
            var dia = fecha.ToString("dd");
            var mes = fecha.ToString("MM");
            ...
            String query = queryBase(año, mes, fechaCompleto) +
                            " select NombreCuenta,Sum (Debe_Dol - Habe_Dol)as total from #tmpAsientos " +
                            " where IdRepGerencia='003' and IdGruGerencia='A00' and IdMes<=" + mes + " " +
                            " group by  " +
                            "      NombreCuenta ";
        ...
        [HttpPost]
        public JsonResult GraficoPrestamos(DateTime fecha)
        {
            var fechaCompleto = fecha.ToShortDateString();
            var dia = fecha.ToString("dd");
            var mes = fecha.ToString("MM");
            ...
            String query = queryBase(año, mes, fechaCompleto) +
                            " select NombreCuenta,Sum (Debe_Dol - Habe_Dol)as total from #tmpAsientos " +
                            " where IdRepGerencia='003' and IdGruGerencia='A20' and IdMes<=" + mes + " " +
                            " group by  " +
                            "     idcuenta,cuenta,NombreCuenta ";
        ...
        [HttpPost]
        public JsonResult GraficoActivos(DateTime fecha)
        {
            var fechaCompleto = fecha.ToShortDateString();
            var dia = fecha.ToString("dd");
            var mes = fecha.ToString("MM");
            ...
            String query = queryBase(año, mes, fechaCompleto) +
                           " select NombreCuenta,Sum (Debe_Dol - Habe_Dol)as total from #tmpAsientos " +
                           "  where IdRepGerencia='003' and IdGruGerencia='A30' and IdMes<=" + mes + " " +
                           "  group by " +
                           "       IDCuenta,Cuenta,NombreCuenta";
        ...
        [HttpPost]
        public JsonResult GraficoAdelantos(DateTime fecha)
        {
            var fechaCompleto = fecha.ToShortDateString();
            var dia = fecha.ToString("dd");
            var mes = fecha.ToString("MM");
            ...
            String query = queryBase(año, mes, fechaCompleto) +
                           " select NombreCuenta,Sum (Debe_Dol - Habe_Dol)as total from #tmpAsientos " +
                           "  where IdRepGerencia='003' and IdGruGerencia='A40' and IdMes<=" + mes + " " +
                           "  group by  " +
                           "       IDCuenta,Cuenta,NombreCuenta";
